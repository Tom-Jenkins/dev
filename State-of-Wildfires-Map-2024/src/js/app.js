...
import { geojsonCountries } from "./Countries";
// import { geojsonBiomes } from "./Continental_Biomes";
// import { geojsonIPCC } from "./IPCC_WGI_regions";
// import { geojsonGADM } from "./GADM_UCDavis_L1";
...
function loadAdditionalData() {
  Promise.all([
    import("./Continental_Biomes.js"),
    import("./IPCC_WGI_regions.js"),
    import("./GADM_UCDavis_L1.js")
  ])
  .then(([module2, module3, module4]) => {
    ...
    // IPCC regions
    const geojsonIPCC = module3.geojsonIPCC;
    geojsonArray.push(geojsonIPCC)
    // GADM
...
// const geojsonArray = [geojsonCountries, geojsonBiomes, geojsonIPCC, geojsonGADM];
...
const robinsonProj = "+proj=robin +lon_0=0 +x_0=0 +y_0=0 +ellps=WGS84 +datum=WGS84 +units=m +no_defs";
...
const simpleMapScreenshoter = L.simpleMapScreenshoter({
  hidden: true,
  hideElementsWithSelectors: [".leaflet-control-zoom", "#btn-reset-view", "#btn-info-modal", "#eu-toggle", "#btn-download-data"],
}).addTo(map);
...
document.getElementById("btn-save-image").addEventListener("click", () => {
  ...
  tempTitle.onAdd = function (map) {
    ...
    this._div.innerHTML = `<h3 style="margin-bottom:0; font-size: 1.5em;">State of Wildfires: Extremes of 2023-2024 Fire Season</h3>`;
...
displayInfo.update = function (property) {
  ...
  if (property) {
    ...
    ${property[element.value] !== null ? property[element.value] > 0 ? `+${property[element.value]}` : property[element.value] : "Insufficient Data"}
...
const ueaLogo = L.Control.extend({
  ...
  onAdd: function() {
    ...
    divContainer.setAttribute("style", "border: none; pointer-events: none; width: 50px; height: 100%; background-color: white; padding: 2px; opacity: 0.80;");
...
// Colour palette
const colPalette = ["#a50026","#f46d43","#fdae61","#fee090","#ffffbf","white","#e0f3f8","#abd9e9","#74add1","#4575b4"];

// Function to style polygons
const stylePolygons = function(variable) {
  return function(feature) {
    return {
      fillColor: colourPolygons(feature.properties[variable], variable, scaleObj, colPalette),
      weight: 0.5,
      opacity: 1,
      color: "black",
      dashArray: "1",
...
const showPopup = function (e) {
  ...
  const popup = L.responsivePopup({ offset: [0, 0] }).setContent(popupContentHtml);
...
const activeLayer = function() {
  ...
  const activeLayerIndex = geojsonArray.map( arr => arr.name ).indexOf(firstSelectorValue);
...
legend.onAdd = function (map) {
  ...
  for (let i = 0; i < grades.length - 1; i++) {
    div.innerHTML += `
      ...
      <span class="legend-text">${grades[i] > 0 ? `+${grades[i]}` : grades[i]}</span>
      <br>
    `;
  }
...
document.getElementById("second-selector").addEventListener("change", () => {
  ...
  const activeVariableText = document.getElementById("second-selector").textContent;
...
const exportCSV = function() {
  ...
  const keysOrder = ["reg_layer", "reg_ID", "reg_name", "start_yr", "end_yr", "start_mo", "end_mo", "land_cov"];
  ...
  let newHeader = keysHeader.map(element => {
    ...
    if (element === "ba_anom") return "Burned Area Anomaly in 2023-24 Fire Season (%)";
    if (element === "ba_final") return "Burned Area in 2023-24 Fire Season (km^2)";
    if (element === "ba_mean") return "Burned Area Mean Annual (km^2)";
    if (element === "ba_moAf") return "Months After Peak with Above-Average Burned Area";
    if (element === "ba_moBf") return "Months Prior to Peak with Above-Average Burned Area";
    ...
    if (element === "cem_emAn") return "Carbon Emitted Anomaly in 2023-24 Fire Season (%)";
    if (element === "cem_emFl") return "Carbon Emitted in 2023-24 Fire Season";
    if (element === "cem_emMn") return "Carbon Emitted Mean Annual (kg)";
    if (element === "cem_emRk") return "Carbon Emitted Rank in 2023-24 Fire Season";
    if (element === "cem_moAf") return "Months After Peak with Above-Average Carbon Emitted";
    if (element === "cem_moBf") return "Months Prior to Peak with Above-Average Carbon Emitted";
    if (element === "cem_moPk") return "Month with Peak in Carbon Emitted Anomaly";
    if (element === "gfa_95FAP") return "95th Percentile Fire Size Anomaly in 2023-24 Fire Season (%)";
    if (element === "gfa_95FRk") return "95th Percentile Fire Size Rank in 2023-24 Fire Season";
    if (element === "gfa_95Fz") return "95th Percentile Fire Size in 2023-24 Fire Season (km^2)";
    if (element === "gfa_95RAP") return "95th Percentile Rate of Growth Anomaly in 2023-24 Fire Season (%)";
    if (element === "gfa_95RRk") return "95th Percentile Rate of Growth Rank in 2023-24 Fire Season";
    if (element === "gfa_95Ros") return "95th Percentile Rate of Growth in 2023-24 Fire Season (km^2 day^-1)";
    if (element === "gfa_mxFz") return "Maximum Fire Size in 2023-24 Fire Season (km^2)";
    if (element === "gfa_mxRos") return "Maximum Rate of Growth in 2023-24 Fire Season (km^2 day^-1)";
    if (element === "gfa_nF") return "Number of Fires Rank in 2023-24 Fire Season";
    if (element === "gfa_nFAnP") return "Number of Fires Anomaly in 2023-24 Fire Season (%)";
    if (element === "gfa_nFRk") return "Number of Fires Rank in 2023-24 Fire Season";
    return element;
  });
...
document.getElementById("btn-download-data").addEventListener("click", exportCSV);


// Function to attach tooltip event listeners
function attachEventListeners() {
  document.querySelector("#legend-title").addEventListener("mouseenter", function() {
      document.querySelector(".legendTooltip").style.display = "flex";
  });

  document.querySelector("#legend-title").addEventListener("mouseleave", function() {
